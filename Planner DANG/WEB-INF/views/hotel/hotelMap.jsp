...
<body>
	...
	<div id="locationField">
		<input id="autocomplete" placeholder="Enter a city" type="search" />
	</div>
	<div id="controls">
		<input type="button" id="autoClick" value="검색"> <input
			type="hidden" id="country" value="all">
	</div>
	<!-- <div id="controls">
      <select id="country">
 ...
 -->
    ...
    function random(){
     ...
     //뷰안에 검색된 결과 반영
        ...
        places.nearbySearch(search, function(results, status) {
          if (status === google.maps.places.PlacesServiceStatus.OK) {
            ...
            for (var i = 0; i < resultNum; i++) {
     		...
     		getInfo(placeI,i);
              google.maps.event.addListener(markers[i], 'click', showInfoWindow);  //마커 클릭시 정보뜬다.
      ...
      function getInfo(placeI,i) {
          places.getDetails({placeId: placeI},function(place, status) {
                if (status != google.maps.places.PlacesServiceStatus.OK) {
                  return;
                }
                ...
                console.log(i+'번째 ='+place.vicinity);
                document.getElementById('info').innerHTML += '<hr><br>호텔이름 : ' + place.name + '<br>'
                					+'호텔 주소 : '+place.vicinity+'<br>'
                					+'<img src="'+photos[0].getUrl({'maxWidth': 60, 'maxHeight': 60})+'">';
              });
      }
	...
	<script
		src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDxUhUBvVgaihAutbHR2OsXEdgnL0di-G0&libraries=places&callback=initMap"
		async defer></script>
	<!-- &language=ko -->
