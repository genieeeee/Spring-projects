...
<script>
function idCheckToServer(){
   
	var alphaCheck=0;
  
	
   for(var i=0; i<document.join.mem_id.value.length; i++){
      if(document.join.mem_id.value.charAt(i) >= 0 && document.join.mem_id.value.charAt(i) <= 9){
         
      }else if(document.join.mem_id.value.charAt(i) >='a' && document.join.mem_id.value.charAt(i) <= 'z'){
   ...
   if(alphaCheck!=0){
      ...
      document.getElementById("idCertResult").innerHTML='※ 알파벳과 숫자 이외의 문자는 사용하실 수 없습니다.';
      document.join.mem_id.value='';
      document.join.idCheckResult.value='n';
   
   }else if(document.join.mem_id.value.length<5){
	      document.getElementById("idCertResult").innerHTML='※아이디는 5자 이상이어야 합니다.';
	      document.join.mem_id.value='';
	      document.join.idCheckResult.value='n';
   }else{
  
      var params = "mem_id="+document.join.mem_id.value;
      sendRequest("idCheck.pd",params,idCheckFromServer,"POST");
   
  }
}
function idCheckFromServer(){
   if(XHR.readyState == 4){
      if(XHR.status == 200){
     
         var data=XHR.responseText;
         if(data=='y'){
            document.getElementById("idCertResult").innerHTML='사용 가능한 아이디입니다.';
            document.join.idCheckResult.value=data;
         }else{
            document.getElementById("idCertResult").innerHTML='이미 사용중인 아이디입니다.';
            document.join.idCheckResult.value=data;
...
function submitForm(){
      //회원가입약관
       ...
       if(document.all.mem_id.value == ''){
          $('#divId').removeClass("has-success");
           $('#divId').addClass("has-error");
           $('#divId').focus();          
       }else if(document.all.idCheckResult.value !='y'){
          $('#divId').removeClass("has-success");
           $('#divId').addClass("has-error");
           $('#divId').focus();
       }else if(document.all.mem_pwd.value != document.all.mem_pwd2.value){
          ...
          document.join.action="memJoin.pd";
...
<div class="page-container">
	<div class="content">
		...
		<br><br>
            <p id="rcorners2" align="center">회원가입</p><br><br><br>

        <form class="form-horizontal" name="join" role="form">
   
            <div class="form-group" id="divId">
               
                <label for="inputId" class="col-lg-2 control-label" style="text-align: left;">아이디</label>
                <div class="col-lg-10">
                    <input type="text" class="form-control onlyAlphabetAndNumber" id="id" name="mem_id" placeholder="5~20자이내의 알파벳, 숫자만 입력 가능합니다." maxlength="20" onchange="idCheckToServer()">
                    <div id="idCertResult" style="color: red;"></div>
                </div>
                
            </div>
            <div class="form-group" id="divPassword">
                <label for="inputPassword" class="col-lg-2 control-label" style="text-align: left;">비밀번호</label>
                <div class="col-lg-10">
                    <input type="password" class="form-control" id="password" name="mem_pwd" placeholder="비밀번호는 8자 이상입니다." maxlength="30">
                </div>
            </div>
            <div class="form-group" id="divPasswordCheck">
                <label for="inputPasswordCheck" class="col-lg-2 control-label" style="text-align: left;">비밀번호 확인</label>
                <div class="col-lg-10">
                    <input type="password" class="form-control" id="passwordCheck" name="mem_pwd2" placeholder="동일한 비밀번호를 입력해주세요." maxlength="30">
                </div>
            </div>
            <div class="form-group" id="divName">
                <label for="inputName" class="col-lg-2 control-label" style="text-align: left;">이름</label>
                <div class="col-lg-10">
                    <input type="text" class="form-control onlyHangul" id="name" name="mem_name" placeholder="이름" maxlength="15">
                </div>
            </div>
           ...
           <div class="form-group" id="divAddr22">
            ...
            <div class="form-group" id="divAgree">
                ...
                <div class="col-lg-10" id="provisionDiv">
                <input type="text" class="form-control" id="provision" name="provision" placeholder="내용보기" readonly="readonly" onclick="showProvision()" style="text-align: center;">
...
③해외에 주소나 거소가 있는 '회원' 의 경우 '회사'와 '회원'간 발생한 분쟁에 관한 소송은 전항에도 불구하고 대한민국 서울중앙지방법원을 관할 법원으로 합니다.
           ...
           <input type="hidden" name="idCheckResult">
            <br>
            <div id="container">
			<div class="button-4" style="margin-left: 180px;">
  		    <div class="eff-4"></div>
    			<a href="javascript:submitForm()"> 가입하기 </a>
			</div>
			</div>
