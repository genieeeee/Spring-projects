...
nhn.FindReplace = jindo.$Class({
    ...
    _findNextElement: function(b) {
        if (!b || b.tagName == "BODY") {
            return null
        }
        var a = b.nextSibling;
        if (a) {
            return (a.nodeType == 1 ? a : this._findNextElement(a))
        } else {
            return this._findNextElement(b.parentNode)
        }
    },
    ...
    findNext: function(g, f, b, a) {
        ...
        if (this.window.find) {
            var k = false;
            return this.window.find(g, f, b, k, a)
        }
        if (this.document.body.createTextRange) {
            try {
                ...
                if (this.document.selection) {
                    ...
                    var d = this._findNextElement(this.window.getSelection().focusNode);
                    if (d) {
                        this._range.moveToElementText(d)
                    }
                }
                c = this._range.findText(g, 1, j);
                this._range.select();
                return c
            } catch (h) {
                return false
        ...
        if (this.document.body.createTextRange) {
            if (this.document.selection) {
                ...
                if (this.window.getSelection) {
                    ...
                    a.removeAllRanges();
                    a.selectAllChildren(this.document.body);
                    a.collapseToStart()
                }
            }
        }
...
nhn.husky.HuskyCore = jindo.$Class({
    ...
    _exec: function(c, b, a) {
        return (this._exec = this.htOptions.oDebugger ? this._execWithDebugger : this._execWithoutDebugger).call(this, c, b, a)
    },
    ...
    registerBrowserEvent: function(e, f, b, d, a) {
        d = d || [];
        var c = (a) ? jindo.$Fn(this.delayedExec, this).bind(b, d, a) : jindo.$Fn(this.exec, this).bind(b, d);
        ...
        if (b.match(/^\$(BEFORE|ON|AFTER)_MSG_APP_READY$/)) {
            ...
            for (d = 0; d < e; d++) {
                if (!f[d]._husky_bRun) {
                    ...
                    if (typeof f[d].$LOCAL_BEFORE_FIRST == "function" && this._execHandler(f[d], "$LOCAL_BEFORE_FIRST", [b, c]) === false) {
                        continue
                    }
                }
                if (typeof f[d].$LOCAL_BEFORE_ALL == "function") {
                    if (this._execHandler(f[d], "$LOCAL_BEFORE_ALL", [b, c]) === false) {
                        continue
                    }
                }
                if (this._execHandler(f[d], b, c) === false) {
    ...
    _execHandler: function(c, b, a) {
        return (this._execHandler = this.htOptions.oDebugger ? this._execHandlerWithDebugger : this._execHandlerWithoutDebugger).call(this, c, b, a)
    },
...
nhn.husky.SE2M_Toolbar = jindo.$Class({
    ...
    _setRoundedCornerButton: function() {
        var e, g, d, c, a, f, b, h;
        f = jindo.$$(">ul>li[class*=" + this.sUIClassPrefix + "]:only-child", this.elTextTool);
        for (e = 0, g = f.length; e < g; e++) {
            d = f[e];
            c = jindo.$Element(d);
            if (c.hasClass("husky_seditor_ui_text_more") || c.hasClass("husky_seditor_ui_photo_attach") || c.hasClass("husky_seditor_ui_map_attach")) {
                continue
            }
            c.addClass("single_child");
            jindo.$Element(jindo.$$.getSingle("button>span._buttonRound", d)).addClass("tool_bg")
        }
        b = jindo.$$("ul>li[class*=" + this.sUIClassPrefix + "]:first-child", this.elTextTool);
        for (e = 0, g = b.length; e < g; e++) {
            d = b[e];
            c = jindo.$Element(d);
            if (c.hasClass("husky_seditor_ui_fontName") || c.hasClass("husky_seditor_ui_text_more") || c.hasClass("husky_seditor_ui_photo_attach") || c.hasClass("husky_seditor_ui_map_attach") || c.hasClass("single_child")) {
                continue
            }
            c.addClass("first_child");
            jindo.$Element(jindo.$$.getSingle("button>span._buttonRound", d)).addClass("tool_bg")
        }
        h = jindo.$$("ul>li[class*=" + this.sUIClassPrefix + "]:last-child", this.elTextTool);
        for (e = 0, g = h.length; e < g; e++) {
            d = h[e];
            c = jindo.$Element(d);
            if (c.hasClass("husky_seditor_ui_fontSize") || c.hasClass("husky_seditor_ui_text_more") || c.hasClass("husky_seditor_ui_photo_attach") || c.hasClass("husky_seditor_ui_map_attach") || c.hasClass("single_child")) {
                continue
            }
            c.addClass("last_child");
            jindo.$Element(jindo.$$.getSingle("button>span._buttonRound", d)).addClass("tool_bg")
        }
    },
...
nhn.husky.SE_EditingArea_WYSIWYG = jindo.$Class({
    ...
    _setBodyHeight: function() {
        ...
        var e = this.getDocument().body,
            b = jindo.$Element(e),
            h = parseInt(b.css("marginTop"), 10) + parseInt(b.css("marginBottom"), 10),
            a = this.oApp.getEditingAreaHeight(),
            d = a - h,
            g = b.height(),
            f, c;
    ...
    setIR: function(h) {
        ...
        if ((this.iframe.contentWindow.document.body.innerHTML).replace(/[\r\n\t\s]*/, "") === "") {
            ...
            this.iframe.contentWindow.document.body.innerHTML = g
        }
...
nhn.husky.SE_PasteHandler = jindo.$Class({
    ...
    _handleBeforePaste: function() {
        ...
        this._sBM_IE = b.placeStringBookmark();
        var c = b.getStringBookmark(this._sBM_IE, true);
        var a = document.createTextNode("");
        this.zwspStart = a.cloneNode(true);
        this.zwspStart.nodeValue = "\u200b";
        this.zwspEnd = this.zwspStart.cloneNode(true);
        c.parentNode.insertBefore(this.zwspEnd, c);
        c.parentNode.insertBefore(this.zwspStart, this.zwspEnd);
        b.setEndBefore(this.zwspEnd);
        b.collapseToEnd();
        b.select()
    },
    _handlePaste: function(c) {
        ...
        setTimeout(jindo.$Fn(function() {
            ...
            if (!!this.zwspStart && !!this.zwspStart.nodeValue) {
                this.zwspStart.nodeValue = this.zwspStart.nodeValue.replace(m, "");
            ...
            try {
                ...
                if (typeof(JEagleEyeClient) != "undefined") {
                    ...
                    var j = "hp_SE_PasteHandler.js/_handlePaste";
                    if (j) {
                        var h = "http://blog.naver.com/";
                        h += j;
                        j = h
                    }
                    JEagleEyeClient.sendError(l, j, f)
                }
            }
            var k = this.oApp.getSelection();
            this._moveToStringBookmark(k);
            k.collapseToEnd();
            k.select();
            this._removeStringBookmark(k)
        }, this).bind(), 0)
    },
    ...
    _saveOriginalContents: function() {
        ...
        if (!this._sBM_IE) {
            this._sBM = b.placeStringBookmark();
            var c = b.getStringBookmark(this._sBM, true);
            var d = b.getStringBookmark(this._sBM);
            var a = document.createTextNode("");
            this.zwspStart = a.cloneNode(true);
            this.zwspStart.nodeValue = "\u200b";
            this.zwspEnd = this.zwspStart.cloneNode(true);
    ...
    _savePastedContents: function() {
        ...
        if (a != "") {
            ...
            this._sTarget = this._filterPastedContents(a)
        }
    },
    _moveToStringBookmark: function(a) {
        if (!!this._sBM_IE) {
            a.moveToStringBookmark(this._sBM_IE)
        } else {
            a.moveToStringBookmark(this._sBM)
        }
    },
    ...
    _filterPastedContents: function(ai) {
        ...
        var an, U, aj = [],
            r = -1,
            q, w = "",
            ac = ["BLOCKQUOTE", "DD", "DIV", "DL", "FORM", "H1", "H2", "H3", "H4", "H5", "H6", "HR", "OL", "P", "TABLE", "UL", "IFRAME"],
            v = new RegExp("^(" + ac.join("|") + ")$", "i"),
            z = false,
            aA = 0,
            h = 0,
            k = [],
        ...
        var I = true,
            ...
            aF = true,
            au = ["BR", "IMG", "COL", "HR"],
            ag = new RegExp("^(" + au.join("|") + ")$", "i"),
            X, d = ["SCRIPT", "STYLE"],
            at = (d.length > 0) ? new RegExp("^(" + d.join("|") + ")$", "i") : null,
            e = false,
            V = false,
            o = false,
            aK = 0,
            f = [],
            W = (f.length > 0) ? new RegExp("^(" + f.join("|") + ")$", "i") : null,
            F = false,
            ad = false,
            j = false,
            Z = 0;
                        ...
                        if (am.test(ai)) {
                            ...
                            if (az.test(ai)) {
                                ...
                                var c = "",
                                    ...
                                    x = false,
                                    H = false,
                                    ab = /([^\w-])(width[\s]*=[\s]*)(["']?)([A-Za-z0-9.]+[%]?)(["']?)/i,
                                    aI = /([^\w-])(height[\s]*=[\s]*)(["']?)([A-Za-z0-9.]+[%]?)(["']?)/i,
                                    ah = /(["';\s])(width[\s]*:[\s]*)([A-Za-z0-9.]+[%]?)([;]?)/i,
                                    l = /(["';\s])(height[\s]*:[\s]*)([A-Za-z0-9.]+[%]?)([;]?)/i;
                                if (this.htBrowser.chrome || this.htBrowser.firefox || (this.htBrowser.safari && this.htBrowser.version >= 6)) {
                                    ...
                                    if (a && a.length > 0) {
                                        for (var T = 0, ak = a.length; T < ak; T++) {
                                            var aw = a[T];
                                            var S = aw + "[\\n\\r\\t\\s]*{([^}]*)}";
                                            var aq = new RegExp(S);
                                            var al = "";
                                            if (aq.test(this._sStyleFromClipboard)) {
                                                al = this._sStyleFromClipboard.match(aq)[1]
                                            }
                                            if (al) {
                                                if (!!aC.test(an)) {
                                                    an = an.replace(aC, "$1$2" + al + " $3$4")
                                                } else {
                                                    an = an.replace(/(>)/g, ' style="' + al + '"$1')
                                                }
                                            }
                                        }
                                    }
                                ...
                                if (ah.test(an)) {
                                    ...
                                    if (x) {
                                        an = an.replace(ah, "$1$2" + g + ";")
                                    }
                                } else {
                                    if (x) {
                                        if (g) {
                                            if (!!aC.test(an)) {
                                                an = an.replace(aC, "$1$2width:" + g + "; $3$4")
                                            } else {
                                                an = an.replace(/(>)/g, ' style="width:' + g + ';"$1')
                                            }
                                        }
                                    }
                                }
                                ...
                                if (l.test(an)) {
                                    ...
                                    if (H) {
                                        an = an.replace(l, "$1$2" + aE + ";")
                                    }
                                } else {
                                    if (H) {
                                        if (aE) {
                                            if (!!aC.test(an)) {
                                                an = an.replace(aC, "$1$2height:" + aE + "; $3$4")
                                            } else {
                                                an = an.replace(/(>)/g, ' style="height:' + aE + ';"$1')
                                            }
                                        }
                                    }
                                }
                                ...
                                if (/^TABLE$/i.test(q)) {
                                    if (u) {
                                        if (!/__se_tbl/g.test(u)) {
                                            var aJ = /(class[\s]*=[\s]*["'])([^"']*)(["'])/i;
                                            var Y = /(class[\s]*=[\s]*)([^"'\s>]+)/i;
                                            if (aJ.test(an)) {
                                                an = an.replace(aJ, "$1$2 __se_tbl_ext$3")
                                            } else {
                                                if (Y.test(an)) {
                                                    an = an.replace(Y, '$1"$2 __se_tbl_ext"')
                                                }
                                            }
                                        }
                                    } else {
                                        an = an.replace(ap, '$1 class="__se_tbl_ext"')
                                    }
                                    z = true;
                                    b = false;
                                    R++
                                    ...
                                    if (/^COL$/i.test(q)) {
                                        Q = an.match(ah)[3];
                                        var P = 0;
                                        if (/span[\s]*=[\s]*"([\d]+)"/.test(an)) {
                                            P = an.match(/span[\s]*=[\s]*"([\d]+)"/)[1]
                                        }
                                        if (!!E[R] && typeof(E[R].length) === "number") {
                                            k = E[R]
                                        } else {
                                            k = []
                                        }
                                        ...
                                        if (/^TR$/i.test(q)) {
                                            ...
                                            if (/^(TD|TH)$/i.test(q)) {
                                                var N = /(:[\s]*)(.5pt|0.5pt)/gi;
                                                var ar = /([^:\d])(.5pt|0.5pt)/gi;
                                                an = an.replace(N, "$11px").replace(ar, "$11px");
                                                if (!(ah.test(an))) {
                                                    k = E[R];
                                                    if (!!k && k.length > 0) {
                                                        var D = k[aA];
                                                        if (D) {
                                                            if (!!aC.test(an)) {
                                                                an = an.replace(aC, "$1$2width:" + k[aA] + "; $3$4")
                                                            } else {
                                                                an = an.replace(/([^\s])([\s]*>|>)/, '$1 style="width:' + k[aA] + ';"$2')
                                                            }
                                                        }
                                                    }
                                                }
                                                if (!(l.test(an))) {
                                                    if (!!L) {
                                                        if (!!aC.test(an)) {
                                                            an = an.replace(aC, "$1$2height:" + L + "; $3$4")
                                                        } else {
                                                            an = an.replace(/([^\s])([\s]*>|>)/g, '$1 style="height:' + L + ';"$2')
                                                        }
                                                    }
                                                }
                                                aA++
                                            } else {
                                                if (/^COLGROUP$/i.test(q)) {
                                                    b = true
                                                } else {
                                                    if (/^TBODY$/i.test(q)) {
                                                        if (b) {
        ...
        if (aH == 0) {
            var af = new RegExp("^<" + this.sParagraphContainer + ">");
            aj[0] = aj[0].replace(af, "")
        }
        return aj.join("")
    },
    ...
    _loadToPasteHelper: function() {
        ...
        if (!this._isPastedMultiParagraph) {
            if (this.htBrowser.firefox) {
                if (/<br>/i.test(this._aGoesPreviousParagraph.join(""))) {
                    c = true;
                    p = this._aGoesPreviousParagraph;
                    this._aGoesPreviousParagraph = [];
                    var q = [];
                    var d = -1;
                    var o = "";
                    var a = false;
                    var m = false;
                    for (var h = 0, l = p.length; h < l; h++) {
                        var b = p[h];
                        m = /<br>/i.test(b);
                        if (h == 0) {
                            if (m) {} else {
                                this._aGoesPreviousParagraph.push(b)
                            }
                        } else {
                            a = /<br>/i.test(o);
                            if (a) {
            ...
            var k = document.createElement("DIV");
            k.innerHTML = this._sTarget;
            j = k.childNodes
        }
        try {
            if (this._aGoesPreviousParagraph.length > 0) {
                var g = this._aGoesPreviousParagraph.join("");
                var k = document.createElement("DIV");
                k.innerHTML = g;
                var p = k.childNodes;
                for (var h = 0, l = p.length; h < l; h++) {
                    this.elPasteHelper.appendChild(p[h].cloneNode(true))
                }
                if (!c) {
                    if (j.length > 0) {
                        if (!!j.splice) {
            ...
            if (j.length > 0) {
                for (var h = 0, l = j.length; h < l; h++) {
                    this.elPasteHelper.appendChild(j[h].cloneNode(true))
                }
            }
        } catch (n) {
            throw n
    ...
    _loadToBody: function() {
        var g = this.oApp.getSelection();
        try {
            this._moveToStringBookmark(g);
            g.select();
            var p = g.getNodes();
            var c = this._markMatchedElementIndex(p, this.aConversionTarget);
            var n = this.oApp.getSelection(this.oApp.getWYSIWYGWindow().parent);
            n.setStartBefore(this.elPasteHelper.firstChild);
            n.setEndAfter(this.elPasteHelper.lastChild);
            n.select();
            var f = n.getNodes();
            var m = this._markMatchedElementIndex(f, this.aConversionTarget);
            if (c.length > 0 && c.length == m.length) {
                for (var h = 0, j = c.length; h < j; h++) {
                    var o = c[h];
                    var d = m[h];
                    var b = p[o];
                    var l = f[d];
                    b.parentNode.replaceChild(l.cloneNode(true), b)
                }
            }
            this._moveToStringBookmark(g);
            g.collapseToEnd();
            ...
            g.select();
            g.deleteContents();
            var a = this._getStringBookmark(g, true);
    ...
    _markMatchedElementIndex: function(d, g) {
        var b = [];
        var f = g.join("|");
        var h = new RegExp("^(" + f + ")$", "i");
        for (var e = 0, a = d.length; e < a; e++) {
            var c = d[e];
            if (h.test(c.nodeName)) {
                b.push(e)
            }
        }
        return b
    },
    _getStringBookmark: function(b, c) {
        var a;
        if (!!this._sBM_IE) {
            a = b.getStringBookmark(this._sBM_IE, c)
        } else {
            a = b.getStringBookmark(this._sBM, c)
        }
        return a
    },
    _removeStringBookmark: function(a) {
        if (!!this._sBM_IE) {
            if (a.getStringBookmark(this._sBM_IE)) {
                a.removeStringBookmark(this._sBM_IE)
            }
        } else {
            if (a.getStringBookmark(this._sBM)) {
                a.removeStringBookmark(this._sBM)
            }
        }
    }
});
nhn.husky.SE_WYSIWYGEnterKey = jindo.$Class({
    ...
    _wrapBlock: function(k, m) {
        var c = this.oApp.getSelection(),
            ...
            d = o.oEnd,
            l, e, h;
        if (!n.bParentBreak || c.rxBlockContainer.test(n.oLineBreaker.tagName)) {
            k.stop();
            c.deleteContents();
            if (!!n.oNode.parentNode && n.oNode.parentNode.nodeType !== 11) {
                l = this.oApp.getWYSIWYGDocument().createElement(this.sLineBreaker);
                c.moveToBookmark(j);
                c.setStartBefore(n.oNode);
                this._addBlankText(c);
                c.surroundContents(l);
                c.collapseToEnd();
                e = this.oApp.getWYSIWYGDocument().createElement(this.sLineBreaker);
                c.setEndAfter(this._getBlockEndNode(n.oNode, d.oNode));
                this._addBlankText(c);
        ...
        if (this.htBrowser.firefox) {
            ...
            if (this.htBrowser.ie) {
                ...
                c.removeStringBookmark(j);
                b = (f.tagName === "U" || nhn.husky.SE2M_Utils.findAncestorByTagName("U", f) !== null);
                a = (f.tagName === "S" || f.tagName === "STRIKE" || (nhn.husky.SE2M_Utils.findAncestorByTagName("S", f) !== null && nhn.husky.SE2M_Utils.findAncestorByTagName("STRIKE", f) !== null));
                if (b || a) {
                    setTimeout(jindo.$Fn(this._addTextDecorationTag, this).bind(b, a), 0);
                    return
                }
                setTimeout(jindo.$Fn(this._addExtraCursorHolder, this).bind(f), 0)
            } else {
                c.removeStringBookmark(j)
            }
        }
...
nhn.husky.SE2M_ExecCommand = jindo.$Class({
    ...
    $BEFORE_EXECCOMMAND: function(a, j, g, f) {
        ...
        if (a.match(/^bold|underline|italic|strikethrough|superscript|subscript$/i)) {
            this.oUndoOption = {
    ...
    _fixAlign: function(d) {
        var f = this.oApp.getSelection(),
            ...
            c = null,
            b = null;
        var h = !this.oNavigator.ie ? function() {} : function(j) {
...
nhn.husky.SE2M_FontNameWithLayerUI = jindo.$Class({
    ...
    _addNanumFont: function() {
        ...
        var c = unescape("%uB098%uB214%uACE0%uB515%uCF54%uB529");
        ...
        if (!!this.elSeparator) {
            this.elSeparator.style.display = d ? "block" : "none"
        }
    },
    ...
    irToDb: function(k) {
        ...
        for (var r = 0, m = d.length; r < m; r++) {
            ...
            try {
                ...
                u.innerHTML = (b + u.innerHTML).replace(/(&nbsp|\s)?(((?!http:\/\/)www\.(?:(?!\&nbsp;|\s|"|').)+)|(http:\/\/(?:(?!&nbsp;|\s|"|').)+))/ig, this._generateAutoLink);
...
nhn.husky.SE2M_LineStyler = jindo.$Class({
    ...
    getTextNodes: function(a, b) {
        var c = function(d) {
            if ((d.nodeType == 3 && d.nodeValue != "\n" && d.nodeValue != "") || (d.tagName == "LI" && d.innerHTML == "") || (d.tagName == "P" && d.innerHTML == "")) {
                return true
            } else {
                return false
            }
...
nhn.husky.SE_WYSIWYGStyler = jindo.$Class({
    ...
    _addCursorHolder: function(c, f) {
        ...
        e = b;
        if (g === d.outerHTML()) {
            a = c._document.createTextNode(this._sZWSP);
            e.appendChild(a);
            return
        }
...
nhn.husky.SE2M_Quote = jindo.$Class({
    ...
    _wrapBlock: function(D, H) {
        var y, q, u, x, c = /BODY|TD|LI/i,
            l, A, d, g, j, B, a, z, K, f, b, r, G, h, J, C;
        this.oApp.exec("RECORD_UNDO_BEFORE_ACTION", ["BLOCK QUOTE", {
            sSaveTarget: "BODY"
        }]);
        y = this.oApp.getSelection();
        if (y.startContainer === y.endContainer) {
            if (y.startContainer.nodeType === 1 && y.startContainer.tagName === "P") {
                if (nhn.husky.SE2M_Utils.isBlankNode(y.startContainer) || nhn.husky.SE2M_Utils.isFirstChildOfNode("IMG", y.startContainer.tagName, y.startContainer) || nhn.husky.SE2M_Utils.isFirstChildOfNode("IFRAME", y.startContainer.tagName, y.startContainer)) {
                    q = y.getLineInfo(true)
                } else {
                    q = y.getLineInfo(false)
                }
            } else {
                var s = y.startContainer;
                var k = y.startOffset;
                var F = y.endContainer;
                var E = y.endOffset;
                var m = y.placeStringBookmark();
                var e = y.getStringBookmark(m);
                var n = e.parentNode;
                var w = nhn.husky.SE2M_Utils.findAncestorByTagName("P", n);
                var v = ["TD", "BODY"];
                if (!w) {
                    for (var G = 0, o = v.length; G < o; G++) {
                        w = nhn.husky.SE2M_Utils.findAncestorByTagName(v[G], n);
                        if (w) {
                            break
                        }
                    }
                    ...
                    var I = w.childNodes;
                    for (var G = 0, o = I.length; G < o; G++) {
                        t.appendChild(I[0])
                    }
                    w.appendChild(t)
                }
                y.removeStringBookmark(m);
                y = this.oApp.getSelection();
                y.setStart(s, k);
                y.setEnd(F, E);
                q = y.getLineInfo(true)
        ...
        u = q.oStart;
        x = q.oEnd;
        if (u.bParentBreak && !c.test(u.oLineBreaker.tagName)) {
            l = u.oNode.parentNode
        } else {
            l = u.oNode
        }
        if (x.bParentBreak && !c.test(x.oLineBreaker.tagName)) {
            A = x.oNode.parentNode
        } else {
            A = x.oNode
        }
        y.setStartBefore(l);
        y.setEndAfter(A);
        d = this._expandToTableStart(y, A);
        if (d) {
            A = d;
            y.setEndAfter(d)
        }
        d = this._expandToTableStart(y, l);
        if (d) {
            l = d;
            y.setStartBefore(d)
        }
        d = l;
        y.fixCommonAncestorContainer();
        g = y.commonAncestorContainer;
        if (y.startContainer == y.endContainer && y.endOffset - y.startOffset == 1) {
            j = y.startContainer.childNodes[y.startOffset]
        } else {
            j = y.commonAncestorContainer
        }
        B = this._findParentQuote(j);
        if (B) {
            B.className = H;
            return
        }
        while (!g.tagName || (g.tagName && g.tagName.match(/UL|OL|LI|IMG|IFRAME/))) {
            g = g.parentNode
        }
        while (d && d != g && d.parentNode != g) {
            d = d.parentNode
        }
        if (d == g) {
            a = g.firstChild
        ...
        z = y._document.createElement(D);
        if (H) {
            z.className = H;
            this._setStyle(z, this.htQuoteStyles_view[H])
        }
        g.insertBefore(z, a);
        y.setStartAfter(z);
        y.setEndAfter(A);
        y.surroundContents(z);
        if (this._isExceedMaxDepth(z)) {
            ...
            b = z.childNodes;
            r = [];
            jindo.$Element(z).leave();
            for (G = 0, h = b.length; G < h; G++) {
                r[G] = b[G]
            }
            for (G = 0, h = r.length; G < h; G++) {
                if (!!K) {
                    jindo.$Element(K).before(r[G])
                } else {
                    jindo.$Element(f).append(r[G])
                }
            }
            return
        }
        y.selectNodeContents(z);
        if (z && z.parentNode && z.parentNode.tagName == "BODY" && !z.nextSibling) {
            J = y._document.createElement("P");
            J.innerHTML = "&nbsp;";
            z.parentNode.insertBefore(J, z.nextSibling)
        }
        if (nhn.husky.SE2M_Utils.isBlankNode(z)) {
        ...
        this.oApp.exec("REFRESH_WYSIWYG");
        setTimeout(jindo.$Fn(function(L) {
            C = L.placeStringBookmark();
            L.select();
            L.removeStringBookmark(C);
            this.oApp.exec("FOCUS")
        }, this).bind(y), 0);
...
nhn.husky.SE2M_TableEditor = jindo.$Class({
    ...
    $ON_TE_MERGE_CELLS: function() {
        ...
        for (e = this.htSelectionSPos.x + 1; e < this.htSelectionEPos.x + 1; e++) {
            ...
            h = curTD;
            b += parseInt(curTD.style.width || curTD.getAttribute("width"), 10)
        }
        ...
        for (e = this.htSelectionSPos.y + 1; e < this.htSelectionEPos.y + 1; e++) {
            ...
            h = curTD;
            a += parseInt(curTD.style.height || curTD.getAttribute("height"), 10)
        }
        if (b) {
            d.style.width = b + "px"
        }
        ...
        d.setAttribute("colSpan", this.htSelectionEPos.x - this.htSelectionSPos.x + 1);
        d.setAttribute("rowSpan", this.htSelectionEPos.y - this.htSelectionSPos.y + 1);
    ...
    $ON_EVENT_EDITING_AREA_MOUSEDOWN: function(c) {
        ...
        switch (this.nStatus) {
            case this.STATUS.S_0:
                ...
                var b = nhn.husky.SE2M_Utils.findAncestorByTagName("TD", c.element);
                if (b && b.tagName == "TD") {
    ...
    _isOnBorder: function(e) {
        ...
        if (e.element.tagName != "TD" && e.element.tagName != "TH") {
            return false
        }
    ...
    _insertColumnAfter: function() {
        ...
        for (var l = 0, o = this.htMap[0].length; l < o; l++) {
            ...
            for (var m = this.htSelectionEPos.x; m >= this.htSelectionSPos.x; m--) {
                ...
                k.insertBefore(b, g);
                if (c.ie && c.nativeVersion >= 9 && g) {
                    g.previousSibling.style.backgroundColor = b.style.backgroundColor
                }
            }
        }
    ...
    _insertRowBelow: function() {
        ...
        this._hideTableTemplate(this.elSelectionStartTable);
        var o;
        var m = this.htMap[0][0].parentNode.parentNode;
        var h = jindo.$$(">TR", m, {
            oneTimeOffCache: true
        });
        var g = h[this.htSelectionEPos.y + 1] || null;
        var c = jindo.$Agent().navigator();
        for (var q = this.htSelectionSPos.y; q <= this.htSelectionEPos.y; q++) {
            o = this._getTRCloneWithAllTD(q);
            m.insertBefore(o, g);
            if (c.ie && c.nativeVersion >= 9) {
                var p = h[this.htSelectionEPos.y].nextSibling.cloneNode();
                m.insertBefore(p, g);
                var k = this.htMap[0][q].parentNode;
                var n = k.childNodes;
                var e = [];
                ...
                for (var f = 0, l = j.length; f < l; f++) {
                    var b = j[f];
                    var a = e[f];
                    if (b.nodeName == "TD" && a && a.nodeName == "TD") {
                        a.style.backgroundColor = b.style.backgroundColor
                    }
                }
            }
        }
        var r = this.htSelectionEPos.x - this.htSelectionSPos.x + 1;
        var d = this.htSelectionEPos.y - this.htSelectionSPos.y + 1;
...
nhn.husky.SE2M_ImgSizeRatioKeeper = jindo.$Class({
    ...
    _isAdjustPossible: function(e) {
        var b = true;
        var a = (this.oApp.htOptions.SE2M_EditingAreaRuler) ? this.oApp.htOptions.SE2M_EditingAreaRuler.bUse : false;
        if (a) {
            ...
            if (d) {
                ...
                if (e > c) {
                    b = false;
                    alert(this.oApp.$MSG("SE2M_ImgSizeRatioKeeper.exceedMaxWidth").replace("#EditorWidth#", c))
                }
            }
        }
        return b
